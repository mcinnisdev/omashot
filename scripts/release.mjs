...
sh(`git tag -a v${next} -m "Omashot v${next}"`);
sh("git push origin main --follow-tags", { stdio: "inherit" });

console.log(`\nv${next} tagged and pushed. Watch the build: gh run watch --repo mcinnisdev/omashot`);
console.log(`Release page: https://github.com/mcinnisdev/omashot/releases/tag/v${next}`);
