---
description: "Omashot Bundles: capture screenshots in groups with notes, auto-capture a process, then export a document or copy a prompt that an AI agent can work from."
---

# Bundles for agents

A bundle is a folder: screenshots in groups, a note on each, and a markdown file that reads in order. It is built for handing to an AI agent, whether that is a bug list to fix or a set of steps to turn into documentation. This is the Omashot Bundles half of the tray. For one-off fixes, see [quick shots](/docs/quick).
...
`Ctrl+Shift+3` auto-captures a region instead: while you do something, Omashot takes a still at the start, at every click or Enter (with a ring where the click landed), and at the end. Press it again to stop, and the bundle window opens on the sequence. Each still is an ordinary shot: drop the noise, note the keepers, drag any that landed out of order, and mark up or blur what needs it. Use this for a process; use single screenshots for faults.
...
- **Your saved prompts** appear in the same menu by name. Add them in the **Prompt library…** (tray, or Hand off in the bundle window) as bundle prompts, with the same placeholders.
