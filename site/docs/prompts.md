---
description: "Edit the text Omashot puts on your clipboard. Six built-in prompts with placeholders, your own quick shot and bundle prompts, and a live preview as you type."
---

# Prompt library

Every text Omashot puts on your clipboard is a template, and the Prompt library is where you edit them and add your own. Open it from **Prompt library…** in the tray, or **Hand off → Prompt library…** in the bundle window.

## Built in

Six texts ship with Omashot. Each can be rewritten, and Reset puts the default back.
...
| Quick shot: a batch | `Ctrl+Enter` with more than one shot in the batch | `{count}`, `{dir}`, `{entries}` |
