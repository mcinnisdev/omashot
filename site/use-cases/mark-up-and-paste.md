...
description: "Add an arrow, highlight a field and pixelate a name before you paste a screenshot. Omashot Basic marks up the shot where it opened and copies it as an image."
---
...
A raw screenshot makes the reader hunt. An arrow on the button, a highlight on the field and the customer's name pixelated out turns it into an instruction. Most screenshot tools do this in a second editor with a second save; in Omashot Basic the tools are already in the window the shot opened in.
...
The marked-up screenshot on the clipboard as an image, ready to paste anywhere that takes one. On disk, the batch folder under `~/Omashot/Quick/` holds two files for the shot:
