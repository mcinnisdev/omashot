...
As a web page it is one self-contained file with the images embedded, ready to send. As Markdown it sits beside the images, ready for a docs platform. The bundle itself stays where it is: a folder under `~/Omashot/` with the screenshots and a `bundle.md` that reads top to bottom.
...
- If there is a logo in `~/Omashot/brand/`, **Export doc** embeds it in the document. Blur in the editor is pixelation, so what you hide is really gone from the exported image.
