...
description: "Record a screen with Omashot Studio and get zooms that follow the cursor, click ripples, keystroke badges, trims and cuts, exported as an H.264 MP4 up to 1440p."
---
...
An H.264 MP4, exactly as previewed, written into the recording's folder under `~/Omashot/`. Naming the recording names the file and the folder. The preview and the export share one renderer, so what you saw is what you send.
...
- Omashot's own overlays are excluded from capture, so a recording of Omashot in use does not show the tool.
