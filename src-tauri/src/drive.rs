...
//! itself. Off unless `OMASHOT_DRIVE_DIR` is set at start-up. The app then
...
pub fn start(app: AppHandle, dir: PathBuf) {
    ...
    eprintln!("omashot: drive mode on, watching {}", dir.display());
    std::thread::spawn(move || loop {
        ...
        for req in reqs {
            ...
            let done = match outcome {
                ...
                Err(e) => {
                    eprintln!("omashot: drive request {} failed: {e}", req.display());
