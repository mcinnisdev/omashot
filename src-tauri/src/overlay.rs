...
/// Omashot's own, and no gesture needed for the camera preview to play.
#[cfg(windows)]
...
pub fn open_rec_badge(
    ...
    if studio {
        if let Ok(hwnd) = win.hwnd() {
            ...
            if ok == 0 {
                eprintln!("omashot: could not exclude the overlay from capture; it will be in the source");
            }
        }
    }
    Ok(())
...
/// Omashot Studio, on a project folder or on the list of recordings.
